...
namespace Xn65.Csharp.Framework.Design
{
    public class MatrixConverter : MathTypeConverter
    {
        public MatrixConverter()
        {
            ...
            var properties = TypeDescriptor.GetProperties(typeFromHandle);
            var propertyDescriptorCollection = new PropertyDescriptorCollection(new PropertyDescriptor[17]
            {
                properties.Find("Translation", ignoreCase: true)!,
            ...
            propertyDescriptions = propertyDescriptorCollection;
        ...
        public override object? ConvertTo(ITypeDescriptorContext? context, CultureInfo? culture, object? value, Type destinationType)
        {
            ...
            if (destinationType == typeof(InstanceDescriptor) && value is Matrix matrix)
            {
                var constructor = typeof(Matrix).GetConstructor(
                [
                    typeof(float),
                    typeof(float),
                    typeof(float),
                ...
                if (constructor != null)
                {
                    return new InstanceDescriptor(constructor, new object[16]
                    {
