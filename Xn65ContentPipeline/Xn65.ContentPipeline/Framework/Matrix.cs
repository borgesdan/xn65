...
using Xn65.Csharp.Framework.Design;

namespace Xn65.Csharp.Framework
{
    [Serializable]
    ...
    public struct Matrix : IEquatable<Matrix>
    {
        ...
        public static Matrix CreateConstrainedBillboard(Vector3 objectPosition, Vector3 cameraPosition, Vector3 rotateAxis, Vector3? cameraForwardVector, Vector3? objectForwardVector)
        {
            ...
            Vector3 vector3;
            Vector3 result2;
            if (Math.Abs(result) > 0.998254657f)
            {
                if (objectForwardVector.HasValue)
                {
                    ...
                    if (Math.Abs(result) > 0.998254657f)
                    {
                        ...
                        vector3 = ((Math.Abs(result) > 0.998254657f) ? Vector3.Right : Vector3.Forward);
                    }
                }
                else
                {
        ...
        public static void CreateConstrainedBillboard(ref Vector3 objectPosition, ref Vector3 cameraPosition, ref Vector3 rotateAxis, Vector3? cameraForwardVector, Vector3? objectForwardVector, out Matrix result)
        {
            ...
            Vector3 vector3;
            Vector3 result3;
            if (Math.Abs(result2) > 0.998254657f)
            {
                if (objectForwardVector.HasValue)
                {
                    vector3 = objectForwardVector.Value;
                    ...
                    if (Math.Abs(result2) > 0.998254657f)
                    {
                        ...
                        vector3 = ((Math.Abs(result2) > 0.998254657f) ? Vector3.Right : Vector3.Forward);
                    }
                }
                else
                {
        ...
        public static Matrix CreatePerspectiveFieldOfView(float fieldOfView, float aspectRatio, float nearPlaneDistance, float farPlaneDistance)
        {
            ...
            {
                throw new ArgumentOutOfRangeException("fieldOfView", string.Format(CultureInfo.CurrentCulture, FrameworkResources.OutRangeFieldOfView, "fieldOfView"));
            }

            if (nearPlaneDistance <= 0f)
            {
                throw new ArgumentOutOfRangeException("nearPlaneDistance", string.Format(CultureInfo.CurrentCulture, FrameworkResources.NegativePlaneDistance, "nearPlaneDistance"));
            }

            if (farPlaneDistance <= 0f)
            {
                throw new ArgumentOutOfRangeException("farPlaneDistance", string.Format(CultureInfo.CurrentCulture, FrameworkResources.NegativePlaneDistance, "farPlaneDistance"));
            }
